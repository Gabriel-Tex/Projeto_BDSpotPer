    ...
    return consultar("""
        ...
        WHERE NOT EXISTS (
            ...
            INNER JOIN Compoe c 
                ...
                AND f.num_faixa = c.num_faixa 
                AND f.num_disc = c.num_disc
            INNER JOIN Compositor co ON c.compositor = co.codigo
            INNER JOIN PeriodoMusical pm ON co.periodo_musical = pm.codigo
            WHERE p.codigo = pf.playlist
              AND (f.tipo_composicao <> 'Concerto'
                   OR pm.descricao <> 'Barroco')
        )
    """)
